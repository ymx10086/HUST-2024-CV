...
def layercam_result(alexnet, img_path, output_path='./output_layer/'):
    ...
    # 根据预测结果获取LayerCAM结果
    mask1, _, activations = alexnet_layercam(normed_torch_img, class_idx=pred_idx)
    # 根据反预测结果获取LayerCAM结果
    mask2, _, activations = alexnet_layercam(normed_torch_img, class_idx=1 - pred_idx)
...
if __name__ == '__main__':
    ...
    for i in os.listdir(img_path):
        tmp_path = img_path + i
        print(tmp_path)
        layercam_result(alexnet, tmp_path)
