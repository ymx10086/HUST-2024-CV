...
from net import Net
from build_dataset import load_dataset_balanced, load_dataset_ration

# 训练模型, 并返回训练过程中的loss, acc, 用tqdm显示进度条
def train(net, dataloader, optimizer, criterion):
    loss_list = []
    acc_list = []
    for input1, input2, labels in tqdm(dataloader):
    ...
    with torch.no_grad():
        for input1, input2, labels in tqdm(dataloader):
            ...
            for i in range(len(labels)):
                class_correct[labels[i]] += (predicted[i] == labels[i]).sum().item()
                class_total[labels[i]] += 1
    print('Accuracy of the network on the test images: %.2f %%' % (100 * correct / total))
    for i in range(2):
        print('Accuracy of %d: %.2f %%' % (i, 100 * class_correct[i] / class_total[i]))


# 主函数
def main():
    # 加载数据集
    train_dataloader, test_dataloader = load_dataset_ration()
    # train_dataloader, test_dataloader = load_dataset_balanced()
    ...
    # 训练模型
    train_loss_list, train_acc_list, test_loss_list, test_acc_list = train_model(net, train_dataloader, test_dataloader, optimizer, criterion, epoch=4)
